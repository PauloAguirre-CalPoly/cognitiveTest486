...
public class GameData extends PropertyChangeSupport {
  ...
  public void addDeviceData(String str) {
    deviceData.add(str);
  }
  ...
  public void recalculate () {
    ...
    for (int i = 0; i < nDisks; i++) {
      ...
      disks.add(new Disk(x, y, diskWidth, color, (char)alpha));
      alpha = alpha - 1;
    }
    for(Disk disk : disks) {
      towers.get(0).dropDisk(disk);
    }
  }
  public boolean isPlayerWin() {
    if(towers.get(1).diskCount() == 4 || towers.get(2).diskCount() == 4) {
      playerWin = true;
    }
    return playerWin;
  }
